...
process.env.NODE_ENV = process.argv.slice(2)[0] || 'production';
console.log(`你当前所处环境为${process.env.NODE_ENV}`);

var argv = require('yargs').usage('Usage: roadhog buildDll [options]').help('h').argv;
...
function build(argv) {
    ...
    try {
        ...
        console.log(_chalk2.default.red('Failed to parse .roadhogrc.js config.'));
        console.log();
        console.log(e.message);
        process.exit(1);
    }
    console.log(rcConfig.dllPlugin);

    if (!rcConfig.dllPlugin) {
        console.log(_chalk2.default.red('dllPlugin config not found in .roadhogrc.js'));
        process.exit(1);
    }

    appBuild = paths.dllNodeModule;
    config = (0, _applyWebpackConfig2.default)(require('./config/webpack.config.dll')(argv, rcConfig, paths), process.env.NODE_ENV);
